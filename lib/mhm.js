...
function HostManager() {
	...
	this._messageServer = net.createServer(function(c) {
		var data = "";
		c.on("data", function(chunk) {
			data += chunk.toString();
		});

		c.on("end", function(chunk) {
			if(chunk) {
				data += chunk.toString();
			}
			...
			if(d.log) {
				...
				var str = d.log[0];
				d.log = d.log.splice(0, 1, d.hostname);

				console.log.apply(this, "[%s] " + str, d.log)
			}
		});

	});

	this._messageServer.listen(function() {
		self._messageServerPort = self._messageServer.address();
	});
}
...
HostManager.prototype = {
	...
	_httpOnRequest: function(req, res, port) {
		...
		if(typeof this._redirects[hostname] != "undefined") {
			var redirectHost = this._redirects[hostname];

			res.writeHead(302, {
				"Location": "http://" + redirectHost + req.url
			});
			res.end();
			return;
		}
		...
		fs.readFile(hostRuntimePath, function(err, data) {
			if(err) { console.log("Could not load host runtime path."); return; }


			try {
				var hostRuntimeData = JSON.parse(data.toString());
			} catch(err) {
				console.log("Could not parse runtime file for '%s'", hostname);
				return;
			}
	...
	addHost: function(hostname, hostpath, callback) {
		...
		if(settings.main) {
			...
			tmp.file({ template: os.tmpdir() + '/mhm-host-XXXXXX.json' }, function(err, tmpFilePath) {
				...
				command += " \"" + self._messageServer.address() + "\"";

				var child = child_process.exec(commandPrefix + command + commandPostfix, {
					cwd: hostpath
		...
		if(settings.redirect) {
			var redirectHost = settings.redirect;
			this._redirects[hostname] = redirectHost;
		}
	},
