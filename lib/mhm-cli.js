...
addCommand("help", { description: "Displays this." }, function() {
  ...
  for(var commandName in commands) {
    ...
    if(command.data.description) {
      ...
      var rows = Math.ceil(description.length / 30);

      var displayMessage = commandMessages[commandName];

      console.log(commandMessages[commandName] + " ".repeat(largestMessageLength - displayMessage.length + 4) + description.substr(0, 30));
      description = description.substr(30);

      for(var i=1; rows > i; i++) {
        console.log(" ".repeat(largestMessageLength + 4) + description.substr(0, 30));
        description = description.substr(30);
      }
